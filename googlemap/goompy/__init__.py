...
def _pixels_to_degrees(pixels, zoom):
    return pixels * 2 ** (21 - zoom)
                                    #marker
def _grab_tile(lat, lon, zoom, maptype, marker, _TILESIZE, sleeptime):
                                                                                          #marker
    urlbase = 'http://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=%d&maptype=%s&marker=%s&size=%dx%d&format=jpg'
    urlbase += _KEY
                                   #marker
    specs = lat, lon, zoom, maptype, marker, _TILESIZE, _TILESIZE
                                         #marker
    filename = 'mapscache/' + ('%f_%f_%d_%s_%s_%d_%d' % specs) + '.jpg'
...
def _pix_to_lat(k, latpix, ntiles, _TILESIZE, zoom):
    ...
    return math.degrees(math.pi/2 - 2 * math.atan(math.exp(((latpix + _pixels_to_degrees((k-ntiles/2)*_TILESIZE, zoom)) - _EARTHPIX) / _pixrad)))
                                              #marker
def fetchTiles(latitude, longitude, zoom, maptype, marker, radius_meters=None, default_ntiles=4):
    '''
    ...
    for j in range(ntiles):
        ...
        for k in range(ntiles):
            ...
            tile = _grab_tile(lat, lon, zoom, maptype, marker, _TILESIZE, 1./_GRABRATE) #marker
...
class GooMPy(object):
                                                                        #marker
    def __init__(self, width, height, latitude, longitude, zoom, maptype, marker, radius_meters=None, default_ntiles=4):
        '''
    ...
    def _fetch(self):
                                                                                                               #marker   
        self.bigimage, self.northwest, self.southeast = fetchTiles(self.lat, self.lon, self.zoom, self.maptype, self.marker, self.radius_meters)
